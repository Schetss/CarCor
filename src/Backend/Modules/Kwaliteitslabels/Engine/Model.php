...
namespace Backend\Modules\Kwaliteitslabels\Engine;

use Backend\Core\Engine\Model as BackendModel;
use Backend\Core\Engine\Language;

/**
 * In this file we store all generic functions that we will be using in the Kwaliteitslabels module
 *
...
class Model
{
    const QRY_DATAGRID_BROWSE =
        'SELECT i.id, i.naam, UNIX_TIMESTAMP(i.created_on) AS created_on, i.sequence
         FROM kwaliteitslabels AS i
         WHERE i.language = ?
         ORDER BY i.sequence';

    const QRY_DATAGRID_BROWSE_CATEGORIES =
        'SELECT c.id, c.title, COUNT(i.id) AS num_items, c.sequence
         FROM kwaliteitslabels_categories AS c
         LEFT OUTER JOIN kwaliteitslabels AS i ON c.id = i.category_id AND i.language = c.language
         WHERE c.language = ?
         GROUP BY c.id
    ...
    {
        BackendModel::get('database')->delete('kwaliteitslabels', 'id = ?', (int) $id);
    }
    ...
    public static function deleteCategory($id)
    {
        ...
        if (!empty($item)) {
            ...
            $db->delete('kwaliteitslabels_categories', 'id = ?', array((int) $id));
            $db->update('kwaliteitslabels', array('category_id' => null), 'category_id = ?', array((int) $id));
        }
    }
    ...
    public static function existsCategory($id)
    {
        return (bool) BackendModel::get('database')->getVar(
            'SELECT 1
             FROM kwaliteitslabels_categories AS i
        ...
        if ($includeCount) {
            return (array) $db->getPairs(
                'SELECT i.id, CONCAT(i.title, " (",  COUNT(p.category_id) ,")") AS title
                 FROM kwaliteitslabels_categories AS i
                 LEFT OUTER JOIN kwaliteitslabels AS p ON i.id = p.category_id AND i.language = p.language
                 WHERE i.language = ?
                 GROUP BY i.id',
    ...
    /**
     * Get the maximum Kwaliteitslabels sequence.
     *
     * @return int
     */
    ...
    public static function insert(array $item)
    {
        ...
        return (int) BackendModel::get('database')->insert('kwaliteitslabels', $item);
    }
    ...
    public static function insertCategory(array $item)
    {
        ...
        return BackendModel::get('database')->insert('kwaliteitslabels_categories', $item);
    }
